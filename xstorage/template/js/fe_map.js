	...
	return new function() {
		...
		this.refreshSearchMobileSpecials = function()
		{
			...
			if (showEffect === true)
			{
				...
				$('.footer-search-results').effect("pulsate", {times:2}, 1500, function(){
		...
		this.initializeSearchMap = function($map)
		{
			...
			this.searchMap.marker = new google.maps.Marker({
				...
				options:{
					icon: {
						url: "//wsf.xgodev.com/xstorage/template/img/map_marker.png",
			...
			var zoomControl = new this.ZoomControl(zoomControlDiv, me.searchMap.map, '-', function(){fe_map.zoomOut(1, $el)});
			...
			zoomControl = new this.ZoomControl(zoomControlDiv, me.searchMap.map, '+', function(){fe_map.zoomIn(1, $el)});
			zoomControlDiv.index = 1;
			this.searchMap.map.controls[google.maps.ControlPosition.RIGHT_BOTTOM].push(zoomControlDiv);
		...
		this.zoomIn = function(times, $map)
		{
			...
			var map;
			if (typeof($map) == 'object') 		{ map = $map.gmap3('get'); }
			else if (typeof($map) == 'string')	{ map = $($map).gmap3('get'); }
			else 								{ map = $('.map').first().gmap3('get');	}
