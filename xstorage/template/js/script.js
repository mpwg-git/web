...
$(document).ready(function() {
	...
	*/
	...
	// raum bearbeiten
	if (top.P_ID == 30) {
		$('.form-mein-raum input[type!="radio"][type!="hidden"]:visible').first().focus();
	}

	$(document).on('focus', 'input', function() {
		$('.landscape-overlay').addClass('force-hidden-overlay');
	});
	...
	$('#blog-kategorie input[type="checkbox"]').off('change').change(function(){
		$.ajax({
		   ...
		   data: $('#blog-kategorie').serialize() + '&lang=' + top.P_LANG,
		   success: function(response){
		   		$('#blog-page').replaceWith($(response.html));
		   }
		});
	});
	...
	$('input[class="blog-menu-input"]').on('change',function(){
		...
		if(allChecked == false){
			$('input[id="blog-kategorie-all"]').prop('checked', false);
		}else{
...
function handleAddressAutocomplete(inputField, autocomplete) {
	...
	// dont refresh search if just in form
	if ($(inputField).hasClass("in-form")) {
		return;
	}
	...
	/*
	 ...
	 console.log("should center", lat, lng);
	 */
...
function chatwindwoResize() {
	...
	*/
	...
	var chatHeaderOffsetBottom = $('.js-chatheader').offset().top + $('.js-chatheader').innerHeight();
