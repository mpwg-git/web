...
class xkalt
{
	...
	public static function updateLog($id,$info,$phone,$state)
	{
	...
	public static function ajax_addComment()
	{
		$b_id 	= intval($_REQUEST['b_id']);
		self::updateLog($b_id,nl2br($_REQUEST['comment']),($_REQUEST['phone']),($_REQUEST['state']));

		$comment 	= dbx::escape($_REQUEST['comment']);
		$phone 		= dbx::escape($_REQUEST['phone']);
		$state 		= dbx::escape($_REQUEST['state']);

		frontcontrollerx::json_success();
	}

/*
	public static function ajax_addState()
	{
		$b_id 	= intval($_REQUEST['b_id']);
		$state	= ($_REQUEST['selectedStateId']);
	...
	public static function ajax_overview($fn)
	{
		switch ($fn)
		{
			case 'load':
				...
				,(select wz_status from wizard_auto_860 inner join wizard_auto_809 on wizard_auto_860.wz_room_id = wizard_auto_809.wz_id) as wz_status
				,(select wz_log from wizard_auto_860 inner join wizard_auto_809 on wizard_auto_860.wz_room_id = wizard_auto_809.wz_id) as wz_log
				,(select wz_phone from wizard_auto_860 inner join wizard_auto_809 on wizard_auto_860.wz_room_id = wizard_auto_809.wz_id) as wz_phone
