...
";
	...
	******************************************************************************************************************/
	...
	private static function cerateDefaultWizardTable($tableName)
	{
		...
		`wz_static` 		MEDIUMTEXT NOT NULL DEFAULT '' COMMENT 'STATIC JSON',
		`wz_sseo` 			MEDIUMTEXT NOT NULL DEFAULT '' COMMENT 'SSEO JSON',
	...
	public static function checkTableAndCreateIfNotExistsViaAtom($a_id)
	{
		...
		$tableName_publishedNumeric 	= $tableNameNumeric.'_published';
		$tableName_versioningNumeric 	= $tableNameNumeric.'_versioning';
		...
		dbx::query("DROP TABLE IF EXISTS $tableName_published;");
		dbx::query("DROP TABLE IF EXISTS $tableName_publishedNumeric;");
	...
	public static function combatOldTables($table,$siteId)
	{
		...
		if (!dbx::tablePresent($table)) return false;
		...
		$remove = array("wz_tm",'wz_tm_de','wz_tm_en','wz_url','wz_url_pnu_id','wz_id_version','wz_id_version','wz_state','wz_online_start','wz_online_stop','wz_tm_id');

		foreach ($cols2kickNow as $search)
		{
			$cols2kick 	= dbx::getAllAttributes($table,$search);
		...
		array(
		'field' => 'wz_static',
		'type'	=> "MEDIUMTEXT NOT NULL COMMENT 'STATIC JSON'",
		'after' => 'wz_sort'
		),

		array(
		'field' => 'wz_sseo',
		'type'	=> "MEDIUMTEXT NOT NULL COMMENT 'SSEO JSON'",
		'after' => 'wz_static'
		),
		...
		dbx::query("ALTER TABLE `$table` ENGINE = MYISAM ");
	...
	public static function translate2basicSQLTyp($as_id,$mode,$old=false)
	{
		...
		switch ($as_type)
		{
			...
			case 'TEXTAREA':
			case 'JSON':
				self::processMultiLangSQL($as," MEDIUMTEXT NOT NULL COMMENT '$comment' ",$old);
				break;
			case 'CHECKBOX':
...
";
	...
	*/
	...
	public static function checkWizardFields($a_id,$combatOldFields=false)
	{
		
		if ($combatOldFields)
		{
			$table = self::genWizardTableNameBy_a_id($a_id);
			self::combatOldTables($table);
		}
		...
		foreach ($fields2add as $att => $type)
		{
		if (!dbx::attributePresent($table,$att))
		{
		$sql = "ALTER TABLE `$table`  ADD `$att` ".$type;
		dbx::query($sql);
		}
		}
		*/
	...
	*/
	...
	public static function wizard_do_magic_grid($function,$extraExtendExtFunctions=false)
	{
		...
		if (is_numeric($_REQUEST['domagic']))
		{
			...
			$fields = array('wz_id','wz_sort','wz_sort_tree','wz_online','wz_created','wz_createdBy');
