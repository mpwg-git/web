...
class xredaktor_xr_html
{
	...
	public static function toStaticHtml($xr_html,$lang=false)
	{
		if ($lang === false) $lang = xredaktor_pages::getFrontEndLang();

		$key = 'xredaktor_xr_html_v3_'.md5($xr_html.$lang);
		$cached = cachex::get($key);

		if ($cached !== false) return $cached;

		$xr_html = self::transformImages($xr_html,$lang);
		$xr_html = self::transformLinks($xr_html,$lang);

		cachex::set($key,$xr_html);

		return $xr_html;
	}

	public static function transformImages($xr_html,$lang)
	{
	...
	public static function resolveWizardVanityUrlSettings($wz_id,$r_id,$lang)
	{

		//$link = wzx_static::getInfopoolLink()
	...
	public static function transformLinks($xr_html,$lang)
	{
		if (trim($xr_html) == "") return $xr_html;
			...
			if (strpos($url,$tagger)!==false)
			{
				...
				switch ($sel)
				{
					case 'tel':
						...
						$tel_number 	= trim($cfg['tel']['tel_number']);
						$tel_title 		= $cfg['tel']['tel_title'];
						$tel_target		= $cfg['tel']['tel_target'];
						...
						if ($tel_number == "")
						{
							$links->item($i)->removeAttribute('title');
							$links->item($i)->removeAttribute('target');
							$links->item($i)->removeAttribute('href');
						} else
						{
							$links->item($i)->setAttribute('title',		$tel_title);
							$links->item($i)->setAttribute('target',	$tel_target);
							$links->item($i)->setAttribute('href',		'tel:'.$tel_number);
						}
					...
					case 'external':
						...
						$ext_nofollow	= ($cfg['external']['ext_nofollow'] == 'Y');
					...
					case 'infopool':
						...
						if (($record_id == 0)||($wizard_id == 0))
						{
							...
							$ip			= wzx_static::getInfopoolLink($cfg['infopool'],$lang);
							$useable 	= $ip['useable'];
