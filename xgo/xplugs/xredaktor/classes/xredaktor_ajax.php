	...
	public static function handleAjax()
	{
		...
		switch ($scope) {
			...
			case 'gui':
				...
				xredaktor_gui::handleAjax($function,$param_0,$param_1,$param_2);
			...
			case 'form_atoms':
				...
				$extFunctionsConfig = array(
				'table'				=> 'forms_mappings',
				'sort'				=> 'fm_sort',
				'pid'				=> 'fm_id',
				'fid'				=> 'fm_fid',
				'del'				=> 'fm_del',
				'name'				=> 'fm_type',
				'extraInsert'		=> array('fm_created' => 'NOW()','fm_s_id' => $s_id),
				'extraLoad'			=> " fm_s_id = '$s_id' ",
				'fields'			=> array('fm_id','fm_type','fm_a_id','fm_view_a_id','fm_sort','fm_avail_fe','fm_avail_be'),
				'update'			=> array('fm_id','fm_type','fm_a_id','fm_view_a_id','fm_sort','fm_avail_fe','fm_avail_be'),
				'normalize'	=> array(
				'string'	=> array('fm_type','fm_a_id','fm_view_a_id','fm_avail_fe','fm_avail_be'),
				'int'		=> array('fm_sort'),
				));
...
";
			...
			case 'atoms_settings':
				...
				'fields'	=> array('as_id','as_collection','as_group','as_name','as_type','as_lastmod','as_lastmodBy','as_type_multilang','as_config','as_use4Export','as_useAsHeader','as_useAsHeaderSort','as_gui_mode','as_gui_pos','as_use4Export','as_gui','as_gui_width','as_gui_height','as_label','as_fe_forms_a_id'),
				'update'	=> array('as_name','as_collection','as_group','as_label','as_type','as_type_multilang','as_useAsHeader','as_use4Export','as_useAsHeaderSort','as_gui_mode','as_gui_pos','as_use4Export','as_gui','as_gui_width','as_gui_height','as_name_de','as_name_en','as_name_ru','as_name_fr','as_name_it','as_name_ro','as_fe_forms_a_id'),
				'normalize'	=> array(
				'string'	=> array('as_name','as_collection','as_group','as_label','as_type','as_type_multilang','as_useAsHeader','as_use4Export','as_useAsHeaderSort','as_gui_mode','as_gui_pos','as_use4Export','as_gui','as_gui_width','as_gui_height','as_name_de','as_name_en','as_name_ru','as_name_fr','as_name_it','as_name_ro', 'as_fe_forms_a_id')
				),
				'postHooks' 		=> array(
				'insert'			=> 'xredaktor_wizards::as_insert',
				'move'				=> 'xredaktor_wizards::as_move',
			...
			case 'rolesTages':
				...
				$rt_s_id = frontcontrollerx::senseVarInt('rt_s_id');
			...
			case 'logs':
				...
				$fields = array('buh_id','buh_r_id','buh_wz_id','buh_table','buh_created','buh_created','buh_type','buh_human','buh_diff');
				$update = array();
				$string = array();
				...
				{
					if (dbx::attributePresent('be_users_history',$k))
					{
						$extraLoad[] = " ($k = '$v')";
					}
				}

				if (count($extraLoad)==0)
				{
					$extraLoad = " 1=1 ";
				} else
				{
					$extraLoad = implode(" AND ",$extraLoad);
				}
			...
			case 'pages':
				...
				$fields = array('p_id','p_vid','p_name','p_lastmod','p_lastmodBy','p_isOnline','p_inMenue','p_frameid','p_sort','p_tm_name','p_full_cache');
				$update = array('p_name','p_vid','p_isOnline','p_frameid','p_inMenue','p_full_cache');
				$string = array('p_name','p_isOnline','p_inMenue','p_full_cache');
			...
			case 'css_less':
				{
					...
					$cl_s_id = frontcontrollerx::senseVarInt('cl_s_id');


					$fields = array('cl_id','cl_name','cl_lastmod','cl_lastmodBy','cl_sort');
					$update = array('cl_name','cl_vid');
					$string = array('cl_name');

					$extFunctionsConfig = array(
					'roles_tag'	=> '',
					'deepsearch'=> array('cl_content'),
					'table'		=> 'css_less',
					'sort'		=> 'cl_sort',
					'pid'		=> 'cl_id',
					'fid'		=> 'cl_fid',
					'name'		=> 'cl_name',
					'del'		=> 'cl_del',
					'extraInsert' 	=> array('cl_created' => 'NOW()'),
					'fields'	=> $fields,
					'update'	=> $update,
					'normalize'	=> array(
					'string'	=> $string,
					...
					$extFunctionsConfig['roles_tag'] 	= 'css_less';
					$extFunctionsConfig['extraMove'] 	= " cl_s_id = '$cl_s_id' ";
					$extFunctionsConfig['extraLoad'] 	= " cl_s_id = '$cl_s_id' ";
					$extFunctionsConfig['extraInsert'] 	= array('cl_s_id'=>$cl_s_id);
					...
					switch ($function)
					{
						case 'loadCode':
							...
							$cl_id 	= frontcontrollerx::isInt($_REQUEST['cl_id']);
							$data 	= dbx::query("select * from css_less where cl_id = $cl_id");
						...
						case 'saveCode':
							...
							$dataHistory = dbx::query("select * from css_less where cl_id = $cl_id");
							unset($dataHistory['cl_id']);
							unset($dataHistory['cl_created']);
							$dataHistory['cl_created'] = "NOW()";
							dbx::insert("css_less_history", $dataHistory);
							

							dbx::update("css_less",array('cl_content'=>$content),array('cl_id'=>$cl_id));
							$data = dbx::query("select * from css_less where cl_id = $cl_id");
							
							$feedback = xredaktor_assets::compile_less($cl_id);
							
							if ($feedback !== true)
							{
								frontcontrollerx::json_debug(array('lessc'=>$feedback));
							}
