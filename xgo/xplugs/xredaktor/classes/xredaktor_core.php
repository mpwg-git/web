...
class xredaktor_core
{
	...
	public static function getFacesSites()
	{
		
		$faces = array();
		$faces[0] = array(); // CORE
		
		$sites = dbx::queryAll("select * from sites where s_del='N'");
		foreach ($sites as $s)
		{
			$s_id = intval($s['s_id']);
			
			$facesOfSite = dbx::queryAll("select * from faces,sites_faces where f_id = sf_f_id and sf_del = 'N' and sf_online='Y' order by sf_sort");
			if ($facesOfSite === false) $facesOfSite = array();
			
			$faces[$s_id] = $facesOfSite;
		}
