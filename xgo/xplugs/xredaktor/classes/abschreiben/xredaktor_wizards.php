...
class xredaktor_wizards
{
	...
	public static function toStaticArray($a_id,$r_id)
	{
		...
		foreach ($ass as $as)
		{
			...
			switch ($as_type)
			{
				...
				case 'HTML':
					...
					$tmp = xredaktor_xr_html::getRelUrls($value);
					foreach ($tmp as $urlx)
					{
						if (in_array($urlx,$relUrls)) continue;
						$relUrls[] = $urlx;
					}
					...
					switch ($as_gui_mode)
					{
						case 1:
							...
							if (isset($db['wz_id']))
							{
								...
								list($sql,$field) = xredaktor_gui::getGenericDataSetsLoaderSqlReverse2(intval($as['as_id']),$r_id);

								$foreignId = intval(dbx::queryAttribute($sql,$field));

								$static['w2w_'.$as_name] = array();

								if ($foreignId != 0)
								{
									list($___d,$crap) = self::toStaticArray($wz_id,$foreignId);
									$static['w2w_'.$as_name] = $___d;
								}
						...
						default:
							...
							$static['w2w_l_'.$as_name] = $checkboxes;
				...
				case 'WIZARD':
					...
					$record	= dbx::query("select $titleIt, wz_id as wz_id from $table where wz_del='N' and wz_id = $value");
...
";
	...
	public static function dbFunctions_specialReleations($as_id)
	{
	...
	******************************************************************************************************************/
	...
	public static function processFrontEndFormSecurity()
	{
		...
		$html = "<input type='hidden' value='$key' name='wzActionInvisibleCaptcha' id='wzActionInvisibleCaptcha' class='wzActionInvisibleCaptcha' >";
		return $html;
	}
	...
	public static function mapLanguageFieldsToNormFieldsMulti($a_id,$records)
	{
		...
		{
			$translated[] = self::mapLanguageFieldsToNormFields($a_id,$r);
		}
		return $translated;
	}
	...
	******************************************************************************************************************/
	...
	public static function genTable($a_id)
	{
		return genWizardTableNameBy_a_id($a_id);
	}
	...
	private static function handle_CONTAINER_FILES($as)
	{
		...
		if (!dbx::tablePresent($table_name))
		{
			...
			CREATE TABLE IF NOT EXISTS `$table_name` (
			  ...
			  `wz_fa_item` ENUM('Y','N') NOT NULL DEFAULT  'N' COMMENT 'IS FROM STORAGE',
...
";
	...
	*/
	...
	public function defaultWizardGrid($a_id,$function)
	{
		...
		if (!is_array(($atomSettings=xredaktor_atoms::getById($a_id)))) frontcontrollerx::json_failure('WIZARD_CONFIG_WRONG',800,array('a_id'=>$a_id));
		if ($atomSettings['a_type'] != 'WIZARD')						frontcontrollerx::json_failure('WIZARD_CONFIG_WRONG',800,array('a_id'=>$a_id));
		...
		$fields 	= array('wz_id','wz_sort','wz_online','wz_created','wz_createdBy');
		foreach ($ass as $as)
		{
			$as_id = $as['as_id'];
		...
		switch ($function)
		{
			...
			case 'spreadSave':
				...
				if (libx::isDeveloper())
				{
					...
					//die("ext ".print_r($extFunctionsConfig));
				...
				//frontcontrollerx::json_debug(array($extFunctionsConfig,$id,$fdata));
				...
				{
					if ($fdata['wz_B_ID'] == 0) {
						$fdata['__warning'] = "Daten gespeichert! Achtung: Es wurde kein Betrieb ausgewählt.";
					}
				}
