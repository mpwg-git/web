...
";
	...
	public static function getGridFields($a_id)
	{
		...
		if (isset($_REQUEST['wzListScopeID']))
		{
	...
	public static function getProcessingLang()
	{
		/*
		if (xredaktor_render::isFrontEndConnection())
		{
			return strtoupper(xredaktor_pages::getFrontEndLang());
		}
		*/
				
		return xredaktor_pages::getBackndEndLang();
	}
	...
	public static function getMultiLangFieldName($as)
	{
		...
		{
			return '_'.self::getProcessingLang().'_wz_'.$as['as_name'];
		}
		return 'wz_'.$as['as_name'];
	}

	public static function getTitleItFieldCombi($a_id,$published)
	{
	...
	public static function getTitleItSql($a_id,$published)
	{
		return self::getTitleItFieldCombi($a_id,$published)." as titleIt ";
	}
	...
	public static function getFinalSqlStatement($a_id,$fields=array(),$start=0,$limit=100,$query=false,$filters=false,$asFilters=false,$sorter=false,$defaultNumericQueryAddons=false,$the3musts=false,$asTree=false,$published=false)
	{
		$published = false;
		...
		if (($tmpx = self::getTitleItSql($a_id,$published)) !== false)
		{
			$tmp[] = $tmpx;
		}
		...
		$sql	.= " from $table where ".$whereAll;
		$sqlCnt	.= " from $table where ".$whereAll;
		...
		if (count($asFilters)>0)
		{
			...
			foreach ($asFilters as $as_id => $value)
			{
				...
				if ($as['as_type'] == "WIZARD")
				{
					$xxxas_name = 'wz_'.$as['as_name'];
					$tmp[] = " ($xxxas_name = $value) ";
				}
		...
		// SORT
		if ($sorter !== false && is_array($sorter))
		{
			...
			if ($asTree)
			{
				...
				if (dbx::attributePresent($table,'wz_sort_tree'))
				{
					$sql .= " order by wz_sort_tree ASC";
				}
	...
	public static function load($a_id, $export=false, $asTree=false, $published=false)
	{
		...
		$sqlObj	= self::getFinalSqlStatement($a_id,$fields,$start,$limit,$_REQUEST['_query'],$filters,$asFilters,$sorter,$defaultNumericQueryAddons,$the3musts, $asTree, $published);
		...
		else if ($export === false && $asTree !== false)
		{
			...
			foreach ($nodes as $k => &$v) {
				...
				$v['_is_dir'] 	= dbx::query("select * from ".$table." where wz_fid = ".$v['wz_id']." LIMIT 1") !== false;
