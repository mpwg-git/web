...
class xredaktor_pages
{
	...
	static $cache 			= false;
	static $currentLang 	= Ixcore::init_fail_lang_fe ;
	static $currentLangBE 	= Ixcore::init_fail_lang_be ;
	...
	public static function getValidLangs($siteId=false) // Frontend only
	{
	...
	public static function getBackndEndLang()
	{
		$lang = Ixcore::init_fail_lang_be;

		if (isset($_REQUEST['BE_LANG']))
		{
			$lang = $_REQUEST['BE_LANG'];
		} else {
			@session_start();
			$lang = $_SESSION['BE_LANG'];
		}
		if (in_array($lang,self::getValidBELangs()))
		{
			self::$currentLangBE = $lang;
		} else
		{
			self::$currentLangBE = Ixcore::init_fail_lang_be;
		}

		return self::$currentLangBE;
	}

	public static function setBackEndLang($lang)
	{
		if (!in_array($lang,self::getValidBELangs())) return false;
		self::$currentLangBE = $lang;
		return true;
	}
