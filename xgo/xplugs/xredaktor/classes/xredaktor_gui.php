	...
	public static function handleAjax($function,$param0,$param1,$param2)
	{
		switch ($function)
		{
			...
			case 'saveSSEO':
				...
				$wz_id 		= intval($_REQUEST['id']);
				$wz_r_id 	= intval($_REQUEST['r_id']);
				$p_sseo 	= $_REQUEST['p_sseo'];

				$table = dbx::escape(xredaktor_wizards::genWizardTableNameBy_a_id($wz_id));
				if (!dbx::tablePresent($table)) die('GRILLE');


				$fields2add = array('wz_sseo'=>'text');
				...
				dbx::update($table,array('wz_sseo'=>$p_sseo),array('wz_id'=>intval($wz_r_id)));
