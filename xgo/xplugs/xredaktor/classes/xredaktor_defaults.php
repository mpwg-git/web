...
class xredaktor_defaults
{
	...
	public static function getWorkBenchSettings()
	{
		...
		'faces_sites'	=> xredaktor_core::getFacesSites(),
		);
	}
	...
	public static function getAll($extFunctionsConfig)
	{
		...
		{
			frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['preHooks']['load']);
		}
	...
	public static function getAllByFather($extFunctionsConfig,$fatherId)
	{
		...
		$extraSQL = "";
		if (isset($extFunctionsConfig['extraLoad']) && ($extFunctionsConfig['extraLoad'] != ""))
		{
		...
		if (($extFunctionsConfig['childrenCount']) && (!$doSearch))
		{
			$preSQL .= " , ";
			...
			$preSQL .= " (select count($var_id) from $table axz where axz.$var_fid = origi.$var_id and $delParam = 'N') as children_cnt ";
		}
		...
		if (!$doSearch)
		{
			...
			$sql = "select * $preSQL from ".self::_getTable($extFunctionsConfig)." origi where (".self::_getFId($extFunctionsConfig)." = $fatherId) $extraDel $extraSQL order by $sorter $sorter_dir";
		...
		} else
		{
			...
			switch ($masterTable)
			{
				case 'atoms':
					...
					$q_content[] = " (`$masterTable`.`a_content` LIKE '%$q%') ";
					...
					foreach ($faces as $f)
					{
						$f_id = intval($f['f_id']);
						if ($f_id == 0) continue;
						$q_content[] = " (`$masterTable`.`a_content_".$f_id."` LIKE '%$q%') ";
					}

					$_q[] = " (".implode(" OR  ",$q_content).") ";
			...
			$sql = "select * $preSQL  from ".self::_getTable($extFunctionsConfig)." where $_q $extraDel $extraSQL order by $sorter $sorter_dir";
		...
		foreach ($nodes as $i => $p)
		{
			...
			if (!$doSearch)
			{
				...
				{
					$nodes[$i]['_is_dir'] = dbx::query("select * from ".self::_getTable($extFunctionsConfig)." where $delParam = 'N' and ".self::_getFId($extFunctionsConfig)." = $id LIMIT 1") !== false;
				}
			} else
			{
	...
	public static function getById($extFunctionsConfig,$id,$checkFathers=false) {
		...
		$page = dbx::query("select * $preSQL from ".self::_getTable($extFunctionsConfig)." where ".self::_getPId($extFunctionsConfig)." = $id");
	...
	public static function updateById($extFunctionsConfig, $id, $data, $saveMode=false) {
		...
		//if (isset($extFunctionsConfig['postHooks']['update'])) wegen log
		{
		...
		{
			if (!frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooks']['update'],$id,$data))
			{
				return false;
			}
		}
		...
		xredaktor_log::generic_update($extFunctionsConfig,$id,$newDataRecord,$oldDataRecord);
		...
		if (isset($extFunctionsConfig['postHooks']['update']))
		{
			...
			frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['postHooks']['update'],$id,$newDataRecord,$oldDataRecord,$extFunctionsConfig);
		}
	...
	public static function insertBlank($extFunctionsConfig)
	{
		...
		{
			if (!($data=frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooksMod']['insert'],$data)))
			{
				return false;
			}
		}
		...
		xredaktor_log::generic_insert($extFunctionsConfig,$id,$data);

		if (isset($extFunctionsConfig['postHooks']['insert']))
		{
			frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['postHooks']['insert'],$id,$extFunctionsConfig);
		}
	...
	public static function insertByFatherId($extFunctionsConfig,$fid,$name)
	{
		...
		if (isset($extFunctionsConfig['preHooks']['insert']))
		{
			if (!frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooks']['insert'],$data))
			{
				return false;
			}
		}
	...
	public static function reorderSort($extFunctionsConfig,$id,$ofid,$nfid,$sort)
	{
		...
		if (isset($extFunctionsConfig['preHooks']['move']))
		{
			if (!frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooks']['move'],$id,$nfid,$ofid,$extFunctionsConfig))
			{
				return false;
			}
		}

		$oldRecordBackup = false;
		if (isset($extFunctionsConfig['postHooks']['move']))
		{
			$oldRecordBackup = self::getById($extFunctionsConfig,$id,	false);
		}
		...
		if (isset($extFunctionsConfig['postHooks']['move']))
		{
			...
			$newDataRecordFather 	= self::getById($extFunctionsConfig,$nfid,	false);
			$oldDataRecordFather 	= self::getById($extFunctionsConfig,$ofid,	false);

			frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['postHooks']['move'],$id,$current,$newDataRecordFather,$oldDataRecordFather,$oldRecordBackup,$extFunctionsConfig);
		}
	...
	public static function reorderSortGrid($extFunctionsConfig,$recs,$dropPosition,$dropRec,$recs2)
	{
		...
		if (isset($extFunctionsConfig['preHooks']['sort']))
		{
			foreach ($recs2 as $newSort=>$rID)
			{
				if (!frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooks']['sort'],$rID,$extFunctionsConfig))
				{
					return false;
				}
			}
		...
		foreach ($recs2 as $newSort=>$rID)
		{
			...
			$sortNewById[$rID] = $newSort;
			$sortOldById[$rID] = intval(dbx::queryAttribute("select $var_sort from $var_table where $var_id = $rID",$var_sort));

			dbx::query("update $var_table set $var_sort = $newSort where $var_id = $rID");
		}

		if (isset($extFunctionsConfig['postHooks']['sort']))
		{
			foreach ($recs2 as $newSort=>$rID)
			{
				frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['postHooks']['sort'],$rID,$extFunctionsConfig,$sortNewById[$rID],$sortOldById[$rID]);
			}
		}
	...
	public static function remove($extFunctionsConfig,$ids)
	{
		...
		foreach ($ids as $id)
		{
			...
			if (isset($extFunctionsConfig['preHooks']['remove']))
			{
				if (!frontcontrollerx::safeCallUserFunctionsTriggerFalse($extFunctionsConfig['preHooks']['remove'],$id,$extFunctionsConfig))
				{
					return false;
				}
			}
			...
			xredaktor_log::generic_delete($extFunctionsConfig,$id);


			if (isset($extFunctionsConfig['postHooks']['remove']))
			{
				frontcontrollerx::safeCallUserFunctions($extFunctionsConfig['postHooks']['remove'],$id,$extFunctionsConfig);
			}
