Ext.define('Ext.xr.field_geo_point', {
	extend: 'Ext.form.FieldContainer',
	alias: 'widget.xr_field_geo_point',
	config: {
		height: 200,
		border: false,
		guiMode: 'point',
	...
	startGoogleMaps : function()
	{
		...
		var startPoint  = new google.maps.LatLng(48.2053453, 16.355569899999978);
		var position 	= false;
		...
		if ((typeof this.polyValue != 'undefined') && (this.polyValue != ''))
		{

			var points_g = [];
			var points = Ext.decode(this.rawConfig.value,true);

			Ext.each(points,function(item){
				points_g.push( new google.maps.LatLng(item.lat,item.lang));
			},this);

			var superPoly = new google.maps.Polygon({
				paths: points_g,
				strokeColor: "#d0ac0f",
				strokeOpacity: 0,
				strokeWeight: 2,
