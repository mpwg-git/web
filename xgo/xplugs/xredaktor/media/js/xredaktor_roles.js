var xredaktor_roles = (function() {
	var instance = null;
	return new function() {
		this.getInstance = function(config) {
			if (instance == null) {
				instance = new xredaktor_roles_(config);
			}
			return instance;
		}
	}
...
var xredaktor_roles_ = function(config) {
	this.config = config || {};
};

xredaktor_roles_.prototype = {
	...
	getRolesTree: function() {
		...
		this.tree = xframe_pattern.getInstance().genTree({
			...
			forceFit:true,
			xstore: {
				load: 	'/xgo/xplugs/xredaktor/ajax/roles/load',
				update: '/xgo/xplugs/xredaktor/ajax/roles/update',
				insert: '/xgo/xplugs/xredaktor/ajax/roles/insert',
				remove: '/xgo/xplugs/xredaktor/ajax/roles/remove',
				params: {r_s_id:s_id},
				move: 	'/xgo/xplugs/xredaktor/ajax/roles/move',
				pid: 	'r_id',
				fields: fields
			}
		});
		...
		this.tree.on('selectionchange',function(view,selections,options){
			...
			Ext.each(this.defPanels,function(p){
				p._tree.getStore().proxy.extraParams['r_id'] = r_id;
	...
	genericPermissionsPanel: function(cfg)
	{
		...
		var name = cfg.fieldsPrefix+'name';
		if (cfg.fieldName)
		{
			name = cfg.fieldName;
		}
		...
		var fields = [
		{name: name,			text:'Name der Seite',		type:'string', folder: true, editor: {allowBlank: false,xtype: 'textfield'}},
	...
	getSettingsPanel:function() {
		...
		var tags = this.genericPermissionsPanel({
			title: 'Tags',
			recursive: false,
			tag: 'tags',
			fieldsPrefix: 'rt_',
			fieldName: 'rt_name_human',
			iconCls: 'xr_roles_tags'
		});

		this.defPanels = [infopool,fa,pages,frames,atoms,tags];
	...
	getMainPanel : function() {
		...
		var gui = Ext.create('Ext.Panel',{
			layout: 'border',
			border: false,
			items: [this.getRolesTree(),this.getSettingsPanel()]
		});
