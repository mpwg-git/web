...
xredaktor_gui_.prototype = {
	...
	renderFormViaVid : function(cfg)
	{
		...
		if (typeof cfg.disabled == "undefined") 						cfg.disabled = false;
			...
			success: function(json) {
				...
				formpanel.getFormData = function()
				{
					...
					Ext.each(json.finalFieldsFlat,function(d){
						var fName = d.name;
						if (typeof values[fName] == "undefined") return;
				...
				formpanel.setFormData = function(values)
				{
					...
					return formpanel.getForm().setValues(data);
				}
	...
	renderFormViaId_v2 : function(cfg)
	{
		...
		xframe.ajax({
			...
			success: function(json) {
				...
				var panel_sseo = xredaktor_gui.getInstance().renderFormViaVid({
					//iconCls: 'xf_settings',
					scope: this,
					listeners: {
						afterBuildFormPanel: function(fp) {

							var data = Ext.decode(json.raw.wz_sseo,true);
							fp.setFormData(data);
					...
					buttons:[{
						...
						handler:function(btn)
						{
							...
							btn.setDisabled(true);
							panel_sseo.mask("Daten werden gespeichert ...");
							var p_sseo = Ext.encode(panel_sseo.getGui().getFormData());

							xframe.ajax({
								scope:this,
								url: '/xgo/xplugs/xredaktor/ajax/gui/saveSSEO',
								params : {
									id: cfg.id || -1,
									r_id: cfg.r_id || -1,
									p_sseo: p_sseo
				...
				Ext.each(butts,function(d){
					cfg.buttons.push(d);
				},this);


				formpanel = xredaktor_forms.getInstance().finalize(cfg.buttons,json);
				formpanel.title = "Datensatz";

				//formpanel2 = xredaktor_forms.getInstance().finalize([],json.published);
				//formpanel2.title = "Published";


				formpanel.tabIndexAlex = 0;
				panel_sseo.tabIndexAlex = 1;
				panel_history.tabIndexAlex = 2;


				if (typeof this.lastSuperFinkiTab == 'undefined') this.lastSuperFinkiTab = 0;

				var gui_v2 = Ext.widget({
					activeTab: this.lastSuperFinkiTab,
					border:false,
					xtype: 'tabpanel',
					tabPosition: 'left',
					items: [formpanel,panel_history],
					listeners: {
						
						scope: this,
						tabchange: function(tb,tab)
						{
							this.lastSuperFinkiTab = tab.tabIndexAlex;
						}
