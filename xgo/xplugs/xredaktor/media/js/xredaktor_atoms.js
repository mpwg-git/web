...
xredaktor_atoms_.prototype = {
	...
	getAvailableFaces: function()
	{
		return xredaktor.getInstance().masterCfg.faces_sites[xredaktor.getInstance().getCurrentSiteId()];
	},

	setSrcEditors: function(atom)
	{
		
		this.latestAtomLoaded = atom;
		...
		Ext.each(this.getAvailableFaces(),function(c){
			...
			try {
				...
				console.info("setSrcEditors",f_id);
			} catch (e) {
				console.info(e);
				this.zendis[f_id] = c;
				console.info("setSrcEditors Catch",f_id);
			}
	...
	getMainPanel : function(a_type, s_id)
	{
		...
		this.panel_form_fe 		= xredaktor_forms_fe.getInstance().getMainPanel(0,a_type);
		this.panel_form_be		= xredaktor_forms_be.getInstance().getMainPanel(0,a_type);
		...
		this.saveContent = function(f_id)
		{
			...
			xframe.ajax({
				...
				stateless: function(success,json) {
					...
					var trigger = "atom_"+this.currentAtomId;
					var i = parseInt(localStorage.getItem(trigger),10);
					if (isNaN(i)) i = 0;
					i++;
					localStorage.setItem(trigger,i);
					console.info("Trigger",trigger,i);
				}
			});
		...
		if (showWizardSettings)
		{
			items.push(this.panel_form_fe);
			items.push(this.panel_form_be);
			items.push({
				xtype: 'panel',
				html: 'WTF',
				title: 'BE-GRID'
			...
			items.push(this.panel_log);
			items.push(this.wizardSettings);
			...
			listeners: {
				...
				tabchange: function(tb,activeTab) {
					...
					Ext.util.Cookies.set('STORE_ACTIVE_PANEL_CONF_'+a_type,activeTabIndex);
						...
						this.setSrcEditors.call(this,this.latestAtomLoaded);
						
						
				//console.info("...",activeTabIndex);
				}
			}
		});
		...
		{name: 'a_id',		xgroup:'Erweitert', 								text:'ID',	type:'int', 	hidden: false, width: 60},
		{name: 'a_vid',		xgroup:'Erweitert', 								text:'vID',	type:'int', 	hidden: !virtualIdSupport, header: virtualIdSupport, editor:true, width: 60},
		{name: 'a_sort',	xgroup:'Erweitert', 								text:'Sort',		type:'int', 	hidden: true, width: 60},
		{name: 'a_lastmod',	xgroup:'Erweitert', 								text:'Letze  Änderung',	type:'string', 	hidden: true, width: 120}
		];
		...
		loadAtomById = function(a_id,finallyDoWizard) {
			...
			this.panel_form_fe.loadAtom(a_id,a_type);
			this.panel_form_be.loadAtom(a_id,a_type);
			this.panel_log.loadAtom(a_id,a_type);
