...
function placeContainerWrapper(s,cfgDiv)
{
	...
	Ext.create('Ext.Button', {
		id:idxButton,
		text: 'Insert atom into container',
		renderTo:idx2,
		arrowAlign: 'bottom',
		scope:this,
		handler : function(btn)
		{
			var atoms = xframe_pattern.getInstance().genTree({
				...
				bbar_add: [{
					id: 'xc_add_atom',
					disabled:true,
					text: 'Insert at the end',
					handler: function(btn)
					{

						if (!latestAtom)
						{
							xframe.alert('Info','choose atom first!');
							return;
						}
				...
				xstore: {
					...
					pid: 	'a_id',
					fields: [
					{name: 'a_name', text:'Name',	type:'string', folder: true, editor: {
						allowBlank: false,
						xtype: 'textfield'
					}},
					{name: 'a_id',			text:'ID',	type:'int', 	hidden: true},
					{name: 'a_lastmod',		text:'Last modifictaion',	type:'string' , hidden: true}
					]
				}
			});
			...
			win = Ext.create('widget.window', {
				...
				tbar2 : [label,{text:'Refresh',handler:function(){window.location=window.location}}],
				items: [atoms]
			});
				...
				var atomsX = xframe_pattern.getInstance().genTree({
					...
					bbar_add: ['->',{
						...
						handler: function(btn)
						{
							...
							if (!latestAtom2)
							{
								xframe.alert('Info','please choose atom first!');
								return;
							}
					...
					xstore: {
						...
						pid: 	'a_id',
						fields: [
						{name: 'a_name', text:'Atom',	type:'string', folder: true, editor: {
							allowBlank: false,
							xtype: 'textfield'
						}},
						{name: 'a_id',			text:'ID',	type:'int', 	hidden: true},
						{name: 'a_lastmod',		text:'Last modification',	type:'string' , hidden: true}
						]
					}
				});
