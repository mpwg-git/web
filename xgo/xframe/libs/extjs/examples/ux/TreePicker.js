...
Ext.define('Ext.ux.TreePicker', {
	...
	*/
	createPicker: function() {
		...
		picker = new Ext.tree.Panel({
			...
			{
				emptyText: 'Suche...',
				idOfNodes: me.store.idProperty,
				xtype:'xfsearchfield_struct',
				paramName : '_query',
				store: me.store,
				url: me.store.getProxy().url,
				flex: 1,
				extraParams: me.store.getProxy().extraParams
			},{
				flex: 0,
				iconCls: 'xf_del',
				text: 'Löschen',
	...
	*/
	onItemClick: function(view, record, node, rowIndex, e) {
		console.info('onItemClick',record.getPath());
	...
	*/
	onExpand: function() {
		...
		if (!node) {
			if (this.backupData)
			{
				if (this.backupData[value])
				{
					var all = this.backupData[value];
					var finalPath = '/0/'+all.path+'/'+value;
					picker.selectPath(finalPath);
				}
			}
		} else {
	...
	*/
	setValue: function(value) {
		...
		if (!record)
		{
			...
			if (typeof this.remoteFetchSelection == "function") {
				this.remoteFetchSelection(value,function(json){

					if (!this.backupData) {
						this.backupData = {};
					}
					this.backupData[value] = json;
