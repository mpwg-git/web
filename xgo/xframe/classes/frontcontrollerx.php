...
class frontcontrollerx
{
	...
	public static function echox($msg)
	{
	...
	*/
	...
	public static function safeCallUserFunctionsTriggerFalse($fns)
	{
	...
	public static function safeCallUserFunctions($fns)
	{
		if (!is_array($fns)) $fns = array($fns);
		foreach ($fns as $fn)
		{
	...
	public static function object2jsonTree($config,$d)
	{
		...
		if ($config['childrenCount'])
		{
			$config['fields'][] = "children_cnt";
		}
	...
	public static function processExtGrid_update($config,$fn)
	{
		...
		dbx::escape($_REQUEST['field']) => $_REQUEST['value']
		);
