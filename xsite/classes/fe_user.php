...
class fe_user
{
	...
	public static function afterLogin()
	{
		...
		$searchData = $_SESSION['xredaktor_feUser_wsf']['wz_SEARCHDATA'];
		
		$tmpSearchData = json_decode($searchData, true);
		$tmpSearchData['type'] = $user['wz_TYPE'];
		
		$searchData = json_encode($tmpSearchData);

		++$counter;
		dbx::update('wizard_auto_707',array('wz_LOGINCOUNTER'=>$counter, 'wz_SEARCHDATA' => $searchData),array('wz_id'=>$userId));
	...
	public static function sc_getMyUserId()
	{
		$userId = intval(xredaktor_feUser::getUserId());
