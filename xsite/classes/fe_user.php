...
class fe_user {
	...
	public static function create_tmp_anbieter_by_email($email) {
		...
		if ($exists) {
			$userId = $exists ['wz_id'];
			dbx::update ( 'wizard_auto_707', $db, array (
					'wz_id' => intval ( $exists ['wz_id'] ) 
			) );
			$existing = true;
		} else {
			$db ['wz_created'] = 'NOW()';
	...
	public static function ajax_reset_password() {
		...
		if ($status < 0) {
			frontcontrollerx::json_failure ( array (
					'id' => $status 
			) );
		} else {
	...
	public static function reset_password() {
		...
		dbx::update ( $table, array (
				'wz_PASSWORT' => md5 ( $PASSWORT ) 
		), array (
				'wz_id' => $user_id,
				'wz_PASSWORT' => md5 ( $currentPwd ) 
		) );
	...
	public static function afterRegistration() {
		...
		if ($user_id === false)
			frontcontrollerx::json_failure ( "ERROR: User does not exist" );
		...
		dbx::update ( "wizard_auto_707", $update, array (
				'wz_id' => $user_id 
		) );
	}
	...
	public static function getMyRoomData($params) {
		...
		if (isset ( $_REQUEST ['createNew'] ) && $_REQUEST ['createNew'] == 1) {
			if ($myRoomId == false) {
				...
				dbx::insert ( 'wizard_auto_853', array (
						'wz_ROOMID' => $myRoomId,
						'wz_STATUS' => 'TODO' 
				) );
				
				if ($params ['redirect'] === true) {
					$newRoomUrl = xredaktor_niceurl::genUrl ( array (
							'p_id' => 30,
							'm_suffix' => $myRoomId,
							'roomId' => $myRoomId,
							'comingFromRedirect' => 1 
					) );
	...
	public static function getUserData($userId) {
		...
		if ($user ['wz_GEBURTSDATUM'] == '0000-00-00')
			$user ['wz_GEBURTSDATUM'] = '';
	...
	public static function getUserProfile($user) {
		...
		$fieldsNull = array (
				...
				'wz_WGGROESSE_VON',
				'wz_WGGROESSE_BIS',
				'wz_ZIMMERGROESSE_VON',
				'wz_ZIMMERGROESSE_BIS',
				'NEUE_MITBEWOHNER_VON',
				'NEUE_MITBEWOHNER_BIS' 
		);
		$fieldsDate = array (
				'wz_ZEITRAUM_VON',
				'wz_ZEITRAUM_BIS' 
	...
	public static function ajax_delFoto() {
		...
		dbx::update ( "wizard_auto_720", array (
				'wz_del' => 'Y' 
		), array (
				'wz_id' => $fotoId,
				'wz_USERID' => $userId 
	...
	public static function ajax_profileSave() {
		...
		// größe von / bis
		if ((intval ( $update ['wz_WGGROESSE_VON'] ) > intval ( $update ['wz_WGGROESSE_BIS'] )) && (intval ( $update ['wz_WGGROESSE_VON'] ) > 0 && intval ( $update ['wz_WGGROESSE_BIS'] ) > 0)) {
		...
		// mitbewohner
		if ((intval ( $update ['wz_ZIMMERGROESSE_VON'] ) > intval ( $update ['wz_ZIMMERGROESSE_BIS'] )) && (intval ( $update ['wz_ZIMMERGROESSE_VON'] ) > 0 && intval ( $update ['wz_ZIMMERGROESSE_BIS'] ) > 0)) {
		...
		if ($_FILES ['add-image-file'] ['size'] > 10145728) // max. 10 MB
{
		...
		if ($s ['s_dir'] != 'Y')
			frontcontrollerx::html_failure ( 'DIR_NOT_EXISTS', 2 );
		...
		$html = xredaktor_render::renderSoloAtom ( $a_id, array (
				'image' => $imageData,
				'type' => $type,
				'refid' => $refid 
		) );
...
// 		$debug = true;
		...
		xredaktor_storage::delFiles ( array (
				$s_id 
		), true );
		...
		dbx::update ( 'storage', array (
				's_crop_original_s_id' => $s_id,
				's_crop_data' => json_encode ( $cropdata ) 
		), array (
				's_id' => $new_s_id 
		) );
	...
	public static function ajax_toggleFav() {
		...
		if ($present === false) {
			...
			dbx::insert ( $table, array (
					'wz_USERID' => $userId,
					'wz_F_USERID' => $f_userId 
			) );
			$state = true;
	...
	public static function getUserAgespanByRoomId($roomId) {
		...
		if ($useData == 'mitbewohner_n2n') {
			...
			foreach ( $user as $userId ) {
				...
				if ($age < intval ( $ret ['FROM'] ))
					$ret ['FROM'] = $age;
				if ($age > intval ( $ret ['TO'] ))
					$ret ['TO'] = $age;
			}
	...
	public static function ajax_delContent() {
		...
		if ($present === false) {
			...
			switch ($type) {
				...
				case 'chat' :
					dbx::update ( $table, array (
							'wz_MESSAGE' => 'Diese Nachricht wurde gelöscht',
							'wz_DELETED' => 'Y' 
					), array (
							'wz_USERID' => $userId,
							'wz_id' => $id 
	...
	public static function burnMail($email, $pageId, $subject = '', $replacersExtra = false, $attachments = array(), $replyToEmail = false, $replyToName = false) {
		...
		if (count ( $send2 ) > 0) {
			...
			if (trim ( $s_mail_reply_name ) == "")
				$s_mail_reply_name = $s_mail_from_name;
			if (trim ( $s_mail_reply_email ) == "")
				$s_mail_reply_email = $s_mail_from_email;
			
			foreach ( $send2 as $to ) {
				
				$replyTo = array (
						'email' => xredaktor_feUser::getFromMail_EMAIL ( $mailSettings ),
						'name' => xredaktor_feUser::getFromMail_NAME ( $mailSettings ) 
				);
				...
				$mailCfg = array (
						...
						'reply' => $replyTo,
						'bcc' => array (
								'email' => 'xgo@pixelfarmers.at',
								'name' => 'xgo@pixelfarmers.at' 
						),
						'html' => $html,
						'txt' => '',
						'subject' => $subject,
	...
	public static function ajax_doFacebookLoginSimple() {
		...
		if ($presentUser !== false) {
			...
			dbx::update ( "wizard_auto_707", array (
					...
					'wz_id' => intval ( $presentUser ['wz_id'] ) 
			) );
		}
	...
	public static function ajax_doFacebookLogin() {
		...
		if ($presentUser === false) {
			...
			if ($presentEmail === false) {
				...
				dbx::update ( 'wizard_auto_707', $dbUpdate, array (
						'wz_id' => $feu_id 
				) );
			...
			if ($db_user ['wz_TYPE'] == 'biete' && $presentEmail === false) {
				...
				foreach ( $_REQUEST ['frage'] as $key => $value ) {
					...
					if ($feu_id > 0) {
						...
						if ($wgTest === false) {
							...
							dbx::update ( 'wizard_auto_765', $db_frage, array (
									'wz_id' => intval ( $wgTest ['wz_id'] ) 
							) );
						}
					}
				}
				...
				$redirectUrl = xredaktor_niceurl::genUrl ( array (
						'p_id' => 30,
						'm_suffix' => $myRoomId,
						'roomId' => $myRoomId,
						'comingFromRedirect' => 1 
	...
	public static function ajax_doSimpleLogin() {
		...
		if ($checkUser === false) {
			...
			$res = xredaktor_feUser::resendRegistration ( array (
					'triggerByVar' => 'doResend',
					'triggerByVal' => '1' 
			) );
		}
		...
		dbx::insert ( 'wizard_auto_845', array (
				'wz_USERID' => intval ( $presentUser ['wz_id'] ),
				'wz_STATUS' => 'TODO' 
		) );
