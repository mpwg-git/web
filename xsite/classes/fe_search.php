...
class fe_search
{
...

	{
		...
		foreach($results as $k => $v)
		{
			...
			$resultsHtml[] = xredaktor_render::renderSoloAtom($a_id_result_single, array(
				'data' => $assign
			));
		}
...

	{
		...
		// anbieter bekommen standardmÃ¤ÃŸig natÃ¼rlich mitbewohner angezeigt
		// im template hats gepasst ("mitbewohner" = active) aber suchergebnisse zeigen bisher nur rÃ¤ume
		// this is the fix:
		$type = $toSearch['type'];
		if (trim($type) == '')
		{
		...
		*/
		if ($type == "biete")
		{
			...
			{
				dbx::update('wizard_auto_707', $updateUserProfile, array(
					'wz_id' => $userId
				));
		...
		// fix fÃ¼r WEB-301 - leider gibts hier ein bÃ¤umchen wechsel dich spielchen:
		switch ($type)
		{
		case 'suche':
	...
	*/
	...
	else
	{
		...
		// print_r($type); die();
		$html = xredaktor_render::renderSoloAtom(749, array(
			'resultsType' => $noResultsType,
			'searchType' => $type
		));
	}
...

{
	...
	// count(*) fÃ¼r anzahl mitbewohner.. einfach die n2n counten
	...
	// bei $filterIsSet (== fav/blocked gewÃ¤hlt) dÃ¼rfen wir keine Suchparameter benutzen!
	if (!$filterIsSet)
	{
	...
	// allgemein: keine deaktivierten / gelÃ¶schten RÃ¤ume in den suchergebnissen
...

{
	...
	// weitere daten in userfelder speichern fÃ¼r WEB-307
	$addData = array();
	if ($isJson === false)
	{
...

{
	...
	*/
	...
	// seit der umstellung profile -> user/rÃ¤ume waren anbieterprofile in der mitbewohner suche vorhanden
	...
	// deshalb  AND wizard_auto_707.wz_TYPE = 'suche' hinzugefÃ¼gt
	...
	*/
	...
	// brauchen wir das bei der usersuche Ã¼berhaupt? weiter unten ist es auskommentiert...
	...
	if (!$filterIsSet)
	{
		foreach($toSearch as $k => $v)
		{
			...
			case 'date':
				...
				(wz_MIETE_VON >= $val)
				AND
				(wz_MIETE_VON < $vA AND )) OR (wz_MIETE_BIS >= $val)) ";
				// 							$where[]	= " (
	...
	if ($type == 'biete')
	{
		...
		if ($vA > $bA)
		{
			$toSearch['price_from'] = $bA;
			$toSearch['price_to'] = $vA;
		}
		$where[] = " (wizard_auto_707.wz_MIETE_VON <= $vA AND wizard_auto_707.wz_MIETE_BIS >= $vA)";
		$where[] = " wizard_auto_707.wz_MIETE_BIS <= $bA ";
	}
	// allgemein: keine deaktivierten / gelÃ¶schten Profile in den suchergebnissen
