				...
				foreach ( $toSearch as $k => $v ) {
					...
					if (trim ( $v ) == '')
						...
						switch ($k) {
							...
							case 'geschlecht' :
								if ($v == 'M')
									$where [] = " ( wz_COUNT_MITBEWOHNER_F = 0 ) ";
									elseif ($v == 'F')
									$where [] = " ( wz_COUNT_MITBEWOHNER_M = 0 ) ";
									break;
							...
							case 'haustiere' :
								if ($v == 'Y')
									$where [] = " ( wz_HAUSTIERE IN ('Y', 'X') )";
									if ($v == 'N')
										$where [] = " ( wz_HAUSTIERE = 'N' )";
										break;
										
							case 'veggie' :
								if ($v == 'Y')
									$where [] = " ( wz_VEGGIE IN ('Y', 'X') )";
									if ($v == 'N')
										$where [] = " ( wz_VEGGIE = 'N' )";
										break;
							...
							case 'barrierefrei' :
								if ($v == 'Y')
									$where [] = " ( wz_BARRIEREFREI = 'Y' ) ";
									if ($v == 'N')
										$where [] = " ( wz_BARRIEREFREI = 'N' ) ";
										break;
	...
	if ($userId == 0)
		...
		if ($isJson === false) {
			...
			if ($searchData ['price_from'])
				$addData ['wz_MIETE_VON'] = $searchData ['price_from'];
				if ($searchData ['price_to'])
					$addData ['wz_MIETE_BIS'] = $searchData ['price_to'];
					if ($searchData ['range'])
						$addData ['wz_UMKREIS'] = $searchData ['range'];
						if ($searchData ['adresse'])
							$addData ['wz_ADRESSE'] = $searchData ['adresse'];
							if ($searchData ['date'] != '')
								$addData ['wz_ZEITRAUM_VON'] = date ( 'Y-m-d', strtotime ( $searchData ['date'] ) );
...
public static function getSearchDataForUser($userId, $returnJson = false) {
	...
	if ($userId == 0)
		...
		if ($searchData === false)
			return false;
			if ($searchData ['wz_SEARCHDATA'] == '')
				return false;
				
				if ($returnJson === true)
					return $searchData ['wz_SEARCHDATA'];
