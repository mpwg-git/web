...
//////////////////////// Div:
	...
	$key_facts = pq("h1.headline.headline-orange")->text();
	$key_facts = explode(utf8_decode('Â'),$key_facts);
	$key_facts = preg_match('!\d+!', $key_facts[0], $matches);
	$wz_groesse = $matches[0];
	
	
	
	// MIETE
	preg_match_all('!\d+!', $angabenObjekt['Miete'], $matches);
	preg_match_all('!\d+!', $angabenObjekt['Nebenkosten'], $matchesNk);
	preg_match_all('!\d+!', $angabenObjekt['Sonstige Kosten'], $matchesSk);
	
	$wz_miete = intval($matches[0][0]);
	$wz_nk = intval($matchesNk[0][0]);
	$wz_sk = intval($matchesSk[0][0]);
	
	$wz_miete_total = $wz_miete + $wz_nk + $wz_sk;
...
// processRoom('wg-zimmer-in-Muenchen-Glockenbach.5946105.html');
