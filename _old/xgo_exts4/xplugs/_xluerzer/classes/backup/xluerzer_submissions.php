...
class xluerzer_submissions
{
	...
	public static function search($stats=false)
	{
			...
			switch ($k)
			{
				...
				case 'overall':
					...
					*/
					...
					{
						$where[] = " (
					(es_firstname LIKE '$p0' AND es_lastname LIKE '$p1' ) OR ( es_firstname LIKE '$p1' AND es_lastname LIKE '$p0' ) OR ( es_submittedBy LIKE '%$p0 $p1%' OR es_submittedBy LIKE '%$p1 $p0%' ) OR (es_email LIKE '$p0') OR (es_email LIKE '$p1') OR (es_company LIKE '$p1 $p0') OR (es_company LIKE '$p0 $p1') 
					) ";
	...
	public static function loadData()
	{
		...
		$ccopy_norm 	= trim(self::getCredits2CopyNorm($es_id,$es_mediaType_id));
