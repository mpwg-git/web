...
class xluerzer_contacts
{
	...
	public static function bulkChange()
	{
		...
		if ($_REQUEST['selector'] != 'false')
		{
			...
			list($sql_data,$sql_cnt) = self::search(true);

			$selector 	= " SELECT ec_id FROM ( ".str_replace("[GRID_FILTERS]","",$sql_data).") emilia ";
			$cnt 		= dbx::queryAttribute(str_replace("[GRID_FILTERS]","",$sql_cnt),"sql_cnt");


			if ($cnt > $LIMIT)
			{
				frontcontrollerx::json_failure("TOO MUCH CONTACTS SELECTED: LIMIT $LIMIT");
			}
		...
		switch ($type)
		{
			case 'addCats':
				...
				{
					$cleanIds 	= array();
					$datax 		= dbx::queryAll($selector,false);

					foreach ($datax as $i => $d)
					{
						$ec_id = intval($d['ec_id']);
						if ($ec_id == 0) continue;
						$cleanIds[] = $ec_id;
			...
			default:
				...
				if (dbx::attributePresent('e_contacts',$type))
				{
					...
					{
						$sql = "update e_contacts set $type = '$value' where ec_id IN ($selector)";
					}
	...
	public static function insert()
	{
		...
		// get freie kundennummer
		$freeId = dbx::queryAttribute("SELECT MIN(t1.actc_client_id + 1) AS nextID
FROM a_client_to_contact t1
   LEFT JOIN a_client_to_contact t2
       ON t1.actc_client_id + 1 = t2.actc_client_id
WHERE t2.actc_client_id IS NULL", "nextID");

		$freeId = intval($freeId);
				
		// update e_contacts und a_client_to_contact
		dbx::insert("a_client_to_contact", array('actc_client_id' => $freeId, 'actc_contact_id' => $ec_id));
		
		$ec_client_id = intval($freeId);
		
		dbx::update("e_contacts", array("ec_client_id" => $ec_client_id), array("ec_id" => $ec_id));
	...
	public static function doSearchInCategories($query)
	{
		$and			= '';
		$field 			= "acc_category";
		$field_value	= "acc_id";

		$sql_data 	= "select DISTINCT(`$field_value`) as _value, `$field` as _display from a_contact_category where $field LIKE '%$query%' $and";
		$sql_cnt 	= "select count(DISTINCT(`$field_value`)) as sql_cnt from a_contact_category  where $field LIKE '%$query%' $and";
	...
	public static function searchCombo($field)
	{
		...
		switch ($field)
		{
			case 'contactLog':
				...
				break;
			case 'categorySingle':
				self::doSearchInCategories($query);
				break;
			case 'ec_company':
	...
	public static function search($returnSql=false)
	{
		...
		foreach ($searchData as $k => $v)
		{
			...
			switch ($k)
			{
				...
				case 'contact_cats':
					...
					foreach ($cats as $c)
					{
						...
						if ($group == 'group')
						{
							...
							if ($gid != 0)
							{
								$tmp[] = " ec_id in (select actc_contact_id from a_contact_to_category,a_contact_category where actc_category_id = acc_id and acc_categoryGroup_id =  $gid ) ";
							}
					...
					// switch AND / OR verknuepfung fuer cats
					if ($searchData['show_cats_or'] == 1)
					{
						$where[] = " ( ".implode(" OR ",$tmp)." ) ";
					}
					else
				...
				case 'categorySingle':
					...
					$where[] = " ( ec_id in (select actc_contact_id from a_contact_to_category where actc_category_id = $singlecat ) ) ";
					break;
						...
						if ($inputs === 3)
						{
							...
							if (((!$company) && (!$person)) || ($company && $person))
							{
								...
								$where[] = " (
									...
									((ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p1%' ) OR ( ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p0%' ) OR (ec_company LIKE '%$p1%$p0%') OR (ec_firstname LIKE '%$p1%$p0%') OR (ec_lastname LIKE '%$p1%$p0%') OR (ec_firstname LIKE '%$p0%$p1%') OR (ec_lastname LIKE '%$p0%$p1%') OR (ec_company LIKE '%$p0%$p1%') OR (ec_email LIKE '%$p0') OR (ec_email LIKE '%$p1') OR (ec_email LIKE '%$p2')) OR 
									((ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p2%' ) OR ( ec_firstname LIKE '%$p2%' AND ec_lastname LIKE '%$p0%' ) OR (ec_company LIKE '%$p2%$p0%') OR (ec_firstname LIKE '%$p2%$p0%') OR (ec_lastname LIKE '%$p2%$p0%') OR (ec_firstname LIKE '%$p0%$p2%') OR (ec_lastname LIKE '%$p0%$p2%') OR (ec_company LIKE '%$p0%$p2%')) OR 
									((ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p2%' ) OR ( ec_firstname LIKE '%$p2%' AND ec_lastname LIKE '%$p1%' ) OR (ec_company LIKE '%$p2%$p1%') OR (ec_firstname LIKE '%$p2%$p1%') OR (ec_lastname LIKE '%$p2%$p1%') OR (ec_firstname LIKE '%$p1%$p2%') OR (ec_lastname LIKE '%$p1%$p2%') OR (ec_company LIKE '%$p1%$p2%'))  
								...
								if ($company)
								{
									...
									*/
									...
									$where[] = " (
								(ec_company LIKE '%$p1%$p0%') OR (ec_company LIKE '%$p0%$p1%') OR (ec_company LIKE '%$p0%$p2%') OR (ec_company LIKE '%$p2%$p0%') OR (ec_company LIKE '%$p0%$p1%$p2%') OR (ec_company LIKE '%$p1%$p0%') OR (ec_email LIKE '%$p0') OR (ec_email LIKE '%$p1') 
								) ";
								...
								} else
								{
									...
									*/
									...
									$where[] = " (
									((ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p1%' ) OR ( ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p0%' )) OR (ec_email LIKE '%$p0') OR (ec_firstname LIKE '%$p1%$p0%') OR (ec_lastname LIKE '%$p1%$p0%') OR (ec_firstname LIKE '%$p0%$p1%') OR (ec_lastname LIKE '%$p0%$p1%') OR 
									((ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p2%' ) OR ( ec_firstname LIKE '%$p2%' AND ec_lastname LIKE '%$p0%' )) OR (ec_email LIKE '%$p1') OR (ec_firstname LIKE '%$p2%$p0%') OR (ec_lastname LIKE '%$p2%$p0%') OR (ec_firstname LIKE '%$p0%$p2%') OR (ec_lastname LIKE '%$p0%$p2%') OR 
									((ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p2%' ) OR ( ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p2%' ))  OR (ec_email LIKE '%$p2') OR (ec_firstname LIKE '%$p2%$p1%') OR (ec_lastname LIKE '%$p2%$p1%') OR (ec_firstname LIKE '%$p1%$p2%') OR (ec_lastname LIKE '%$p1%$p2%') 
									) ";
							...
							if (((!$company) && (!$person)) || ($company && $person))
							{
								...
								{
									$where[] = " (
									(ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p1%' ) OR ( ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p0%' ) OR (ec_firstname LIKE '%$p1%$p0%') OR (ec_lastname LIKE '%$p1%$p0%') OR (ec_firstname LIKE '%$p0%$p1%') OR (ec_lastname LIKE '%$p0%$p1%') OR (ec_company LIKE '%$p1%$p0%') OR (ec_company LIKE '%$p0%$p1%') OR (ec_email LIKE '%$p0') OR (ec_email LIKE '%$p1')
									) ";
								} else {
									$where[] = " (
									(ec_firstname LIKE '%$p0%') OR ( ec_lastname LIKE '%$p0%') OR (ec_company LIKE '%$p0%') OR (ec_email LIKE '%$p0')
									) ";
								}
								...
								if ($company)
								{
									...
									*/
									...
									{
										$where[] = " (
								(ec_company LIKE '%$p1%$p0%') OR (ec_company LIKE '%$p0%$p1%') OR (ec_email LIKE '%$p0') 
								) ";
									} else {
										$where[] = " (
								ec_company LIKE '%$p0%' OR (ec_email LIKE '%$p0') 
								) ";
									}
								...
								} else
								{
									...
									*/
									...
									{
										$where[] = " (
									(ec_firstname LIKE '%$p0%' AND ec_lastname LIKE '%$p1%' ) OR ( ec_firstname LIKE '%$p1%' AND ec_lastname LIKE '%$p0%' ) OR (ec_firstname LIKE '%$p1%$p0%') OR (ec_lastname LIKE '%$p1%$p0%') OR (ec_firstname LIKE '%$p0%$p1%') OR (ec_lastname LIKE '%$p0%$p1%') OR (ec_email LIKE '%$p0')  
									) ";
									} else {
										$where[] = " (
									(ec_firstname LIKE '%$p0%') OR ( ec_lastname LIKE '%$p0%') OR (ec_email LIKE '%$p0') 
									) ";
									}
		...
		$sql_data = "SELECT e_contacts.*, abu_username, ac_name, (select count(eamc_id) from e_archive_media_credits where ec_id = eamc_contact_id) as credits_total FROM `e_contacts`

		LEFT JOIN a_country 		ON ec_country_id = ac_id
		LEFT JOIN a_backend_user 	ON ec_assignedTo = abu_id
		
		WHERE [GRID_FILTERS] $where ORDER BY `ec_id` asc";


		$sql_cnt = "SELECT count(*) as sql_cnt FROM `e_contacts`
		
		LEFT JOIN a_country 		ON ec_country_id = ac_id
		LEFT JOIN a_backend_user 	ON ec_assignedTo = abu_id
		
		WHERE [GRID_FILTERS] $where ";
