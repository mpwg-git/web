...
class xluerzer_log
{
	...
	public static function pushLog($al_action,$table,$pid,$id,$oldData=false,$al_human='',$al_mods=array())
	{
		//$al_mods = array();
	...
	{
		self::pushLogSimple('LOGIN');
		$user 	= xluerzer_user::getCurrentUser();
		$abu_id = intval($user['abu_id']);
		dbx::query("update a_backend_user set abu_lastLogin = NOW() where abu_id = $abu_id");
	}


	public static function pushLogSimple($al_action,$al_human='')
	{
	...
	{
		print_r($oldDataRecord);
		print_r($newDataRecord);
		$table 	= $extFunctionsConfig['table'];
		$pid 	= $extFunctionsConfig['pid'];
		self::update($table,$pid,$idx,$oldDataRecord);
	}
