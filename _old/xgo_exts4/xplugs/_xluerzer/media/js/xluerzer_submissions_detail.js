...
xluerzer_submissions_detail_.prototype = {
		...
		this.panel_media_print = Ext.widget({
			...
			},
			{
				...
				getUploadUrl: function() {
					return xluerzer.getInstance().getAjaxPath('submissions/uploadHighResPrint/'+this.es_id+'?mod='+this.es_modified);
				},
			...
			},
			{
				scope: this,
				getUploadUrl: function() {
					return xluerzer.getInstance().getAjaxPath('submissions/uploadLowResPrint/'+this.es_id+'?mod='+this.es_modified);
				},
				xtype: 'field_file_logic',
						...
						win = Ext.create('Ext.window.Window', {
							...
							bbar: ['->',{
								...
								handler: function() {
									...
									this.masterTab.mask("Updating Credits ...");
									xframe.ajax({
										scope: this,
										url: xluerzer.getInstance().getAjaxPath('submissions/copySubmissionCredits'),
										params : {
											es_id: this.es_id,
											from_es_id: Ext.getCmp(superCopy).getValue()
										},
						...
						items: [
						{
							...
							autoEl : {
								...
								render: function(c){
									c.getEl().on({
										click: function() {
											var url = xluerzer.getInstance().getAjaxPath('submissions/img_orig/'+me.es_id+'?mod='+this.es_modified);
	...
	saveCredits: function()
	{
		...
		Ext.each(this.panel_credits.items.items,function(inp){
			...
			if (inp.credit_type_internal == 'xcredit')
			{
		...
		*/
		...
		xframe.ajax({
			scope: this,
			url: xluerzer.getInstance().getAjaxPath('submissions/saveCredits2/'+this.es_id),
			params : params,
			stateless: function(success, json)
			{
				this.masterTab.unmask();
	...
	sendSubmissionEmail: function()
	{
		...
		xframe.ajax({
			...
			stateless: function(success, json)
			{
				...
				var infoAssoz = {};
				var data = [];
				Ext.each(json.alternative_mails,function(obj){
					
					infoAssoz[obj.amt_id] = obj;
				...
				var comboMails = xluerzer_gui.getInstance().simplyCombo({
					flex: 1,
					fieldLabel: 'Other mail text',
					name: '',
					value: '',
					data: data,
					emptyText: '',
					...
					listeners: {
						...
						change: function(cbx,value) {
							...
							xframe.ajax({
								scope: this,
								url: xluerzer.getInstance().getAjaxPath('submissions/resendSubmissionMail/'+this.es_id+'?st_or='+value),
								params : {},
								stateless: function(success, json)
								{
									if (!success)
									...
									gui.getForm().setValues({
										'mail_body':json.mail_body,
										'mail_subject':json.mail_subject
									});
								}
							});
	...
	load: function(delta)
	{
		...
		xframe.ajax({
			...
			stateless: function(success, json)
			{
				...
				this.panel_credits.getForm().setValues(json.xcredits);
				...
				this.es_modified = json.submission.es_modified_ts;

				Ext.getCmp(this.btn_highRes).setDisabled(true);
				Ext.getCmp(this.btn_highRes2).setDisabled(true);
				...
				if ((json.submission.es_video_poster_s_id == '0'))
				{
					Ext.getCmp(this.btn_selStill).setDisabled(true);
				}
				
				if ((json.submission.es_video_poster_original_id == '0'))
				{
					Ext.getCmp(this.btn_origStill).setDisabled(true);
				}
				...
				$$('#'+this.btn_normRes2a).attr('href', xluerzer.getInstance().getAjaxPath('submissions/downloadSubmission/'+this.es_id));
				$$('#'+this.btn_highRes2a).attr('href', xluerzer.getInstance().getAjaxPath('submissions/downloadSubmissionHighRes/'+this.es_id));
				console.info("Updateing",this.btn_normRes2a,$$(this.btn_normRes2a));
	...
	{
		var me = this;
		var submission_img_medium_src = xluerzer.getInstance().getAjaxPath('submissions/img_medium/'+this.es_id+'?mod='+this.es_modified);
	...
	getTab_video: function()
	{
		...
		this.btn_selStill = Ext.id();
		this.btn_origStill = Ext.id();
		...
		this.panel_video_right = Ext.widget({
			...
			{
				text: 'Download Full UploadedRes Still',
				id: this.btn_origStill,
				xtype: 'button',
				width: '200px',
				scope: this,
				handler: function() {

					var url = xluerzer.getInstance().getAjaxPath('submissions/downloadOrigStillHigh/'+this.es_id);
					window.open(url,'DOWNLOAD');
			...
			{
				text: 'Download Full VideoRes Still',
				id: this.btn_selStill,
				xtype: 'button',
				width: '200px',
				scope: this,
				handler: function() {
					...
					var url = xluerzer.getInstance().getAjaxPath('submissions/downloadBEStillHigh/'+this.es_id);
					window.open(url,'DOWNLOAD');
