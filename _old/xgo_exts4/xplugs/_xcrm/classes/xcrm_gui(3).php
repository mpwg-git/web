...
class xcrm_gui
{
	...
	public static function html_left()
	{
		...
		type			=> 'b',
		title 			=> 'BETRIEBSDATEN',
		searchfields 	=> array(959,960,961,1167,array('stype'=>'n2n','w_id'=>209,'f_w_id'=>216,'title'=>'KATEGORIE'))
		),
	...
	public static function handleAjaxFunction($function)
	{
		switch ($function)
		{
			...
			case 'bd_search':
				...
				$sql = "select $table_image.* from $table_image,$table_n2n,$table_keys where
				...
				if (count($s_final)==0)
				{
					$sql = "select * from $table_image where 1";
				}
				...
				if ($files !== false) {
					...
					$html = xredaktor_render::renderSoloAtom(286,array('images'=>$final));
			...
			case 'show_bilddatenbank':
				$settings = array('searchfields'=>array(1027,1029));
	...
	public static function extend2genericSearch($master_wz_id)
	{
