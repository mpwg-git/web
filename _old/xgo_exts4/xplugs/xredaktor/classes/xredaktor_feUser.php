...
class xredaktor_feUser
{
	...
	*/
	...
	private static $sessionName_FEUSER 					= 'xredaktor_feUser_';
	private static $siteCallAfterLogin					= "";
	private static $siteCallAfterRegister				= "";
	private static $siteCallAfterSave					= "";
	private static $wzId 								= 180;

	private static $page_login							= 29;
	private static $page_account						= 36;
	private static $page_resendToken					= 41;
	private static $page_resetPwd						= 42;
	private static $page_errorDuplicateEmail			= 49;
	private static $page_checkToken						= 44;

	private static $mailPage_welcomeWithToken			= 32;
	private static $mailPage_welcomeWithToken_subject	= "Registrierung auf xxxx";
	private static $mailPage_resetPwdWithToken			= 33;
	private static $mailPage_resetPwdWithToken_subject	= "Passwort xxxxx neu sezten";
	private static $MAIL_FROM_NAME						= "xgo@pixelfarmers.at";
	private static $MAIL_FROM_EMAIL						= "xgo@pixelfarmers.at";
	...
	private static $userField_type 			= "wz_ANREDE";
	...
	private static $initFieldsDone = false;
	...
	******************************************************************************************/
	...
	private static function reDirect($state)
	{
		switch ($state)
		{
			case (self::$userState_ok):
				...
				{
					$url = frontcontrollerx::safeCallUserFunction($siteCallAfterLogin);
				}
			...
			case (self::$userState_mailCheck):
				header("Location: ".xredaktor_niceurl::genUrl(array('p_id'=>self::$page_resendToken)));
				break;
	...
	public static function doLogin($params)
	{
		...
		$present = dbx::query("select * from $t where wz_EMAIL = '$u' and wz_PASSWORT = MD5('$p') and wz_online = 'Y' and wz_del='N'");
		if ($present === false) return array('state'=>self::$userState_notFound);

		if ($present[self::$userField_userConfirmed] != 'Y') self::reDirect(self::$userState_mailCheck);
	...
	public static function getUserRecordByEMail($email)
	{
		...
		$u 			= dbx::query("select * from $table where $fieldEmail = '$email' and wz_del='N' and wz_online='Y'");
	...
	public static function getNameBy($userId)
	{
	...
	public static function isEmailAlreadyRegistered($email)
	{
		...
		$email 			= dbx::escape(trim($email));
		$emailCnt 		= dbx::queryAttribute("select count(wz_id) as emailCnt from $table where $field_email = '$email' and wz_del='N'",'emailCnt');
		return ($emailCnt >= 1);
	}
	...
	private static function getFromMail_EMAIL($mailSettings)
	{
	...
	private static function getFromMail_NAME($mailSettings)
	{
