	...
	public static function resize($rmode,$file_src,$file_dest,$w,$h,$ext='jpg',$quality=95,$colorspace='',$crop='', $isAnimGif=false)
	{
		...
		switch ($rmode)
		{
			case 'strict':
				...
				exec("$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd -background $background '$file_src' -resize ".$w."x".$h."\> -size ".$w."x".$h." xc:none +swap -gravity center -quality $quality -composite '$file_dest' 2>&1",$out);
			...
			case 'hcut':
				...
				$out = array();
				exec("$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd -background $background $addOnPdf '$file_src' -resize ".$w.$extra_w."x".$h.$extra_h." -gravity center  -extent ".$w."x".$h." -quality $quality  '$file_dest'  2>&1",$out);
				if (!file_exists($file_dest)) {
					return $out;
				}
				break;
			...
			case 'cco':
				...
				$cmd = "$convert  -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd $addOnPdf '$file_src' -resize ".$w."x".$h."^ -gravity center -crop ".$w."x".$h."+0+0 -quality $quality '$file_dest'   2>&1";
			...
			case 'ccowhitebg':
				...
				if (libx::isDeveloper())
				{
						$cmd = "$convert  -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd '$file_src' $alpharemove -resize ".$w."x".$h."^ -gravity center -crop ".$w."x".$h."+0+0 -quality $quality -background $background '$file_dest'   2>&1";
						//die("ja ".$cmd);
				}
			...
			case 'festrict':
				...
				exec("$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd '$file_src' -resize ".$w."x".$h."\> -size ".$w."x".$h."  xc:none +swap -gravity center -quality $quality -composite -flatten -background white '$file_dest' 2>&1",$out);
			...
			default:
				...
				if (($w==-1)||($h==-1))
				{
					...
					if (($w == "") && ($h == ""))
					{
						$cmd = "$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd $addOnPdf '$file_src'  -quality $quality $alpharemove '$file_dest' 2>&1";
					} else
					{
								
						$cmd = "$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd $addOnPdf '$file_src' -resize ".$w."x".$h."  -quality $quality $alpharemove '$file_dest' 2>&1";
				...
				} else
				{
					...
					if ($ext == 'png')
					{
						$cmd = "$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB."  $grayCmd -background $background $addOnPdf '$file_src' -resize ".$w."x".$h."\> -quality $quality  '$file_dest'  2>&1";
					} else
					{
						$cmd = "$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB."  $grayCmd -background $background -alpha remove $addOnPdf '$file_src' -resize ".$w."x".$h."\> -quality $quality  '$file_dest'  2>&1";
					}
				}
		...
		**/
		...
		if (!file_exists($cachedFile)||(!$useCache))
		{
			...
			if ($strict)
			{
				...
				exec("$convert -strip -colorspace ".Ixcore::PATH_ImageMagick_RGB." $grayCmd -background $background $alpharemove '$file' -resize ".$w."x".$h."\> -size ".$w."x".$h." xc:none +swap -gravity center -quality $quality -composite '$cachedFile' 2>&1",$out);
