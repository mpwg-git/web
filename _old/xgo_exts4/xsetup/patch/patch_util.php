...
class patch_util
{
	...
	{
		self::echox("<h2>2) Synching Wizards to Database ...</h2>");
		$wizards = dbx::queryAll("select * from atoms where a_del='N' and a_type='WIZARD'");
		foreach ($wizards as $w)
		{
			$a_id 	= $w['a_id'];
			$a_name	= $w['a_name'];
			self::echox("<h3>$a_name</h3>");
			xredaktor_wizards::checkWizardFields($a_id);
		}
	}

	public static function repairStorage($path)
	{
/*		$path = '/srv/www/pichlarn.at/htdocs';
	...
	public static function doAll($path)
	{
		self::processPatchingByCompleteSqlFile();
		self::syncWizards();
		self::repairStorage($path);
